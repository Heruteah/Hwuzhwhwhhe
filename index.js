...
async function main() {
  ...
  console.log(
    chalk.green.bold('Are you sure you want to launch a DDoS attack?')
  );
  const { confirmDDoS } = await inquirer.prompt({
    type: 'confirm',
    name: 'confirmDDoS',
    message: chalk.green('Confirm (y/n):'),
  });

  if (!confirmDDoS) {
    console.log(chalk.red.bold('The DDoS process has been canceled.'));
    return;
  }
  ...
  const { url } = await inquirer.prompt({
    type: 'input',
    name: 'url',
    message: chalk.green('Enter the target URL for the DDoS attack.:'),
  ...
  const { duration } = await inquirer.prompt({
    type: 'input',
    name: 'duration',
    message: chalk.green('Enter the duration of the DDoS attack (in seconds).:'),
    validate: input => {
      const parsed = parseInt(input);
      return isNaN(parsed) || parsed <= 0
        ? 'Enter a valid duration (a positive number).'
        : true;
    },
  });

  const dataBox = boxen(
    `URL Target: ${chalk.green(url)}\nAttack Duration: ${chalk.green(duration)} seconds`,
    { padding: 1, margin: 1, borderStyle: 'double', borderColor: 'green' }
  );

  console.log(chalk.green.bold('The data you entered:'));
  console.log(dataBox);

  const { confirmData } = await inquirer.prompt({
    type: 'confirm',
    name: 'confirmData',
    message: chalk.green('Is the data you entered correct? (y/n):'),
  });

  if (!confirmData) {
    console.log(chalk.red.bold('Restarting the process from the beginning...'));
    return main();
  }

  const preparingSpinner = ora(
    chalk.green('Preparing for the DDoS attack...')
  ).start();

  setTimeout(() => {
    preparingSpinner.succeed(chalk.green('✓ The DDoS attack has been prepared.'));
    ...
    const progressBar = new cliProgress.SingleBar({
      format:
        ...
        ' | ' +
        chalk.yellow('{value}/{total} Seconds'),
      barCompleteChar: '\u2588',
      barIncompleteChar: '\u2591',
      hideCursor: true,
    });
    ...
    const interval = setInterval(() => {
      ...
      if (value >= parseInt(duration)) {
        ...
        progressBar.stop();
        console.log(
          chalk.red.bold('The DDoS attack has been launched against ' + url)
        );

        setTimeout(() => {
          console.log(
            chalk.green.bold('Process completed. Exiting the application.')
          );
          process.exit(0);
        }, 1000);
      }
